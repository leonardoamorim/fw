...
iptables -A INPUT -p tcp -i enp0s9 -s $EXT -d $FWL1 --dport 52001 -j ACCEPT
...
iptables -A FORWARD -p icmp -s $LNK --icmp-type echo-request -o enp0s9 -j ACCEPT
...
iptables -A FORWARD -p tcp -s $LNK -o enp0s9 -m multiport --dports 80,443 -j ACCEPT
...
for MAIL in 25 110 143 587 993 995
    do
        iptables -t nat -A PREROUTING -p tcp -i enp0s9 -s $EXT -d $FWL2 --dport $MAIL -j DNAT --to-destination $DTC:$MAIL
done

#iptables -A OUTPUT -p tcp -s 0/0 --sport 1024:65535 -d 0/0 --dport 25:995 -j ACCEPT
#iptables -A INPUT -p tcp -s 0/0 --sport 25:995 -d 0/0 --dport 1024:65535 -j ACCEPT
